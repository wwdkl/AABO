...
class AnchorHead(nn.Module):
    ...
    def __init__(self,
        ...
        if self.muti_scales:
            assert len(self.anchor_ratios) == len(self.anchor_scales)
            self.num_anchors_list = [len(self.anchor_scales[i]) for i in range(5)]

            self.anchor_generators = []
            for i in range(len(self.anchor_base_sizes)):
                anchor_base = self.anchor_base_sizes[i]
                self.anchor_generators.append(
                    AnchorGenerator(anchor_base, anchor_scales[i], anchor_ratios[i]))
            self.num_anchors = max(self.num_anchors_list)
        else:
